...
// ─── Segment switcher — card style ────────────────────────────────────────────
type Segment = "personas" | "empresas"

function CardSegmentSwitcher({
  segment,
  onChange,
}: {
  segment: Segment
  ...
  return (
    <div className="inline-flex rounded-xl border border-border bg-card p-1 gap-1" role="tablist" aria-label="Seleccionar segmento">
      {(["personas", "empresas"] as Segment[]).map((s) => (
        <button
          key={s}
          role="tab"
        ...
        >
          {s === "personas" ? "Para mí" : "Para mi empresa"}
        </button>
      ))}
    </div>
  )
...
export function ProductShowcaseV2() {
  ...
  return (
    <section className="bg-background py-20 lg:py-28" aria-labelledby="showcase-v2-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* Card-style segment switcher */}
          <motion.div variants={fadeInUp}>
            <CardSegmentSwitcher segment={segment} onChange={setSegment} />
          </motion.div>
        </motion.div>
